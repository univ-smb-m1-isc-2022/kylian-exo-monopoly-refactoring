...
public class Joueur implements Comparable {
    ...
    public void joue(int total, Case dep, Case imp, Case lux, Case allerenpri, Case priso) {
        ...
        if (allerenpri.equals(position)) { // si il arrive sur aller en prison
            position = priso;
            System.out.println(nom + " est sur Aller en prison. " + getSexeJ()
                    + " va directement en prison sans passer par le départ !!!");
    ...
    public void payerLoyer(CaseConstructible c, ArrayList<CaseConstructible> listecase, ArrayList<Joueur> j) {
        if (!listecase.contains(c) && !casespossedes.contains(c)) { // si la case a un propritaire et que ce n est pas
                                                                    // le joueur
            if (c.getCouleur() == "gare") { // traitement du cas particulier de la gare
                ...
                for (Joueur aJ : j) {
                    if (aJ.casespossedes.contains(c)) {
                        nombregare = aJ.getNombrePropriete(c.getCouleur()) - 1; // recupere le nombre de gare posseder
                                                                                // par le proprietaire de la case gare
                                                                                // -1 pr calcul loyer
                        ...
                        if (argent >= 0) {
                            ...
                            aJ.argent += c.getLoyer() * (int) Math.pow(2, nombregare) + argent; // paye tout ce qui lui
                                                                                                // reste avant de faire
                                                                                                // faillitte
                        }
                        if (argent >= 0) {
                            System.out.println(nom + " paye un loyer de " + montantloyer + " a " + aJ.getNomJ()
                                    + " car " + aJ.getNomJ() + " possede " + (nombregare + 1) + " gares.");
                        } else {
                ...
                for (Joueur aJ : j) {
                    if (aJ.casespossedes.contains(c)) { // cherche le joueur qui est proprietaire de la case
                        if (aJ.getNombrePropriete(c.getCouleur()) == c.nombreProprieteDeLaCouleur(c.getCouleur())) { // si
                                                                                                                     // le
                                                                                                                     // propriatire
                                                                                                                     // a
                                                                                                                     // toutes
                            ...
                            if (argent >= 0) { // si peut payer le loyer
                                ...
                                System.out.println(
                                        nom + " paye un loyer double de " + c.getLoyer() * 2 + " a " + aJ.getNomJ()
                                                + " car " + nom + " possede toutes les proprietes de cette couleur.");
                            ...
                            if (argent >= 0) { // si il paye le loyer
                                ...
                                System.out.println(
                                        nom + " paye un loyer de " + c.getLoyer() + " a " + aJ.getNomJ() + ".");
