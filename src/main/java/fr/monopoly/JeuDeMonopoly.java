...
public class JeuDeMonopoly {
    ...
    private void jouerUnTour(Joueur unjoueur) {
        if (!stop) { // verifier avant le joueur suivant si la partie est arrete
            ...
            unjoueur.monLance(total); // plus logique de l'afficher avant son eventuel deplacement, achat ou paiment
                                      // de loyer, prison j'ai donc decomposé mon ousuisje initial
            // SI DOUBLE
            if (verifdouble) {
                unjoueur.aFaitUnDouble(plateau.prison); // incremente double met rejouer a true, le met en prison ,
                                                        // condition liberable
    ...
    private void jouerLeTotalDe(Joueur unjoueur, int total) {
        unjoueur.joue(total, plateau.depart, plateau.impot, plateau.luxe, plateau.allerenprison, plateau.prison); // tester
                                                                                                                  // si
                                                                                                                  // cas
                                                                                                                  // construtible
    ...
    private void afficheFinDePartie() {
        System.out.println("La partie est terminee !!!"); // L'affichage de la suite etait pas deamnde mais ca parait
                                                          // plus coherent
        trie();
        System.out
                .println("Le vainqueur est " + joueurs.get(0).getNomJ() + " avec " + joueurs.get(0).getArgent() + ".");
        joueurs.get(0).afficherLesProprietes();
        for (int i = 1; joueurs.size() > i; i++) {
            if (joueurs.get(i).getArgent() > 0) {
                System.out.println(joueurs.get(i).getNomJ() + " est " + (i + 1) + " place avec "
                        + joueurs.get(i).getArgent() + ".");
